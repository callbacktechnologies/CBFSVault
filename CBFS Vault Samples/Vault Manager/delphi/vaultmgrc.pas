...
uses SysUtils, Classes, StrUtils, CBVCore, CBVCBVault, CBVConstants, IOUtils, Windows;
...
begin
  Result := Path;
  if (Length(Result) > 0) and (AnsiChar(Result[Length(Result)]) in PathDelimiters) then
...
begin
  ...
  begin
    ...
    if (Result = 0) and OptionDeleteAfterArchiving then
    begin
      if not DeleteFile(PChar(OSFileName)) then
        Result := GetLastError;
    end;
  end;
end;
