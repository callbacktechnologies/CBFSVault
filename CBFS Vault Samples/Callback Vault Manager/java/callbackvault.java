...
class gchandle {
    ...
    private static final Map<String, gchandle> handles = new HashMap<String, gchandle>();
    private static final Map<String, Integer> handleCounts = new HashMap<String, Integer>();
    private static final Object lock = new Object();
    private static final SecureRandom random = new SecureRandom();

    gchandle(Object target) {
        id = new BigInteger(63, random).toString(10);
        this.target = target;
    }

    public static gchandle Alloc(Object target) {
        gchandle handle = findHandleFor(target);
        if (handle == null) {
            handle = new gchandle(target);
        }
        handle.Retain();
        return handle;
    }

    private static gchandle findHandleFor(Object target) {
        for (gchandle handle : handles.values()) {
            if (target == handle.getTarget()) {
                return handle;
            }
        }
        return null;
    ...
    public static String ToIntPtr(gchandle handle) {
        return handle.id.toString();
    }

    public static gchandle FromIntPtr(Object value) {
    ...
    public void Retain() {
        synchronized (lock) {
            ...
            if (!handleCounts.containsKey(id)) {
                handleCounts.put(id, 0);
                handles.put(id, this);
            }
            handleCounts.put(id, handleCounts.get(id) + 1);
        }
    }

    public boolean Free() {
        synchronized (lock) {
            if (!handleCounts.containsKey(id)) throw new RuntimeException("Attempt to free a null reference");

            handleCounts.put(id, handleCounts.get(id) - 1);

            if (handleCounts.get(id) == 0) {
                handleCounts.remove(id);
                handles.remove(id);
                target = null;
                return true;
            }
    ...
    public static String getStatusString() {
        StringBuilder state = new StringBuilder("Handles Status:\n");

        synchronized (lock) {
            for (String id : handleCounts.keySet()) {
                state.append(id);
                state.append(" has ");
                state.append(handleCounts.get(id));
                state.append(" open handles. [");
                state.append(handles.get(id));
                state.append("]\n");
            }
        }
...
public class callbackvault {
    ...
    static private int IterateFiles(String FilePath, Method IterateFunc, boolean Recursive, Cbvault Vault) throws  Exception
    {
        ...
        if (ProceedFilePath)
            Result = (Integer)IterateFunc.invoke(null, FilePath, true, Vault);
    ...
    static private int OpenVault(String VaultName, Cbvault[] VaultArg, boolean ExistingOnly)
    {
        ...
        try
        {
            Vault.setCallbackMode(true);
            Vault.addCbvaultEventListener(new CbvaultEventListener() {
                ...
                public void vaultClose(CbvaultVaultCloseEvent cbvaultVaultCloseEvent) {
                    ...
                    if (cbvaultVaultCloseEvent.vaultHandle != 0)
                    {
                        try {
                            RandomAccessFile stream = (RandomAccessFile) gchandle.FromIntPtr(String.valueOf(cbvaultVaultCloseEvent.vaultHandle)).getTarget();
                            stream.close();
                            gchandle.FromIntPtr(String.valueOf(cbvaultVaultCloseEvent.vaultHandle)).Free();
                        }
                        catch (Exception e)
                        {
                            System.out.println(String.format("vaultClose : %s", e.getMessage()));
                            cbvaultVaultCloseEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                        }
                    }
                }
                ...
                public void vaultDelete(CbvaultVaultDeleteEvent cbvaultVaultDeleteEvent) {
                    ...
                    File file = new File(cbvaultVaultDeleteEvent.vault);
                    try {
                        file.delete();
                    }
                    catch (Exception e)
                    {
                        System.out.println(String.format("vaultDelete : %s", e.getMessage()));
                        cbvaultVaultDeleteEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                    }
                }

                public void vaultFlush(CbvaultVaultFlushEvent cbvaultVaultFlushEvent) {

                    cbvaultVaultFlushEvent.resultCode = 0;
                }

                public void vaultGetParentSize(CbvaultVaultGetParentSizeEvent cbvaultVaultGetParentSizeEvent) {
                    File file = new File(cbvaultVaultGetParentSizeEvent.vault);
                    cbvaultVaultGetParentSizeEvent.freeSpace = file.getFreeSpace();
                }

                public void vaultGetSize(CbvaultVaultGetSizeEvent cbvaultVaultGetSizeEvent) {
                    try {
                        RandomAccessFile stream = (RandomAccessFile) gchandle.FromIntPtr(String.valueOf(cbvaultVaultGetSizeEvent.vaultHandle)).getTarget();
                        cbvaultVaultGetSizeEvent.size = stream.length();
                        cbvaultVaultGetSizeEvent.resultCode = 0;
                    }
                    catch (Exception e)
                    {
                        System.out.println(String.format("vaultGetSize : %s", e.getMessage()));
                        cbvaultVaultGetSizeEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                    }
                }

                public void vaultOpen(CbvaultVaultOpenEvent cbvaultVaultOpenEvent) {

                    File file = new File(cbvaultVaultOpenEvent.vault);
                    try {
                        boolean truncate = false;
                        if (cbvaultVaultOpenEvent.openMode == Constants.VAULT_OM_CREATE_NEW) {
                            if (!file.exists())
                                file.createNewFile();
                            else {
                                cbvaultVaultOpenEvent.resultCode =Constants.VAULT_ERR_VAULT_FILE_ALREADY_EXISTS;
                                return;
                            }
                        }
                        else
                        if (cbvaultVaultOpenEvent.openMode == Constants.VAULT_OM_CREATE_ALWAYS) {
                            if (!file.exists())
                                file.createNewFile();
                            else {
                                truncate = true;
                        ...
                        if (cbvaultVaultOpenEvent.openMode == Constants.VAULT_OM_OPEN_EXISTING) {
                            if (!file.exists())
                            {
                                cbvaultVaultOpenEvent.resultCode = Constants.VAULT_ERR_VAULT_FILE_DOES_NOT_EXIST;
                                return;
                            }
                        }
                        else
                        if (cbvaultVaultOpenEvent.openMode == Constants.VAULT_OM_OPEN_ALWAYS) {
                            if (!file.exists())
                                file.createNewFile();
                        }

                        RandomAccessFile stream = new RandomAccessFile(file, "rwd");
                        if (truncate)
                            stream.setLength(0);
                        cbvaultVaultOpenEvent.vaultHandle = Long.parseLong(gchandle.ToIntPtr(gchandle.Alloc(stream)));
                        cbvaultVaultOpenEvent.resultCode = 0;
                    }
                    catch (FileNotFoundException fnfe)
                    {
                        System.out.println(String.format("vaultOpen : %s", fnfe.getMessage()));
                        cbvaultVaultOpenEvent.resultCode = Constants.VAULT_ERR_VAULT_FILE_DOES_NOT_EXIST;
                        cbvaultVaultOpenEvent.vaultHandle = -1;
                    }
                    catch (IOException e)
                    {
                        System.out.println(String.format("vaultOpen : %s", e.getMessage()));
                        cbvaultVaultOpenEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                        cbvaultVaultOpenEvent.vaultHandle = -1;
                    }
                }

                public void vaultRead(CbvaultVaultReadEvent cbvaultVaultReadEvent) {
                    RandomAccessFile stream = (RandomAccessFile) gchandle.FromIntPtr(String.valueOf(cbvaultVaultReadEvent.vaultHandle)).getTarget();
                    try {
                        stream.seek(cbvaultVaultReadEvent.offset);
                        byte[] buff = new byte[cbvaultVaultReadEvent.buffer.capacity()];
                        cbvaultVaultReadEvent.count = stream.read(buff);
                        if (cbvaultVaultReadEvent.count == -1)
                            cbvaultVaultReadEvent.count = 0;
                        else
                            cbvaultVaultReadEvent.buffer.put(buff, 0, cbvaultVaultReadEvent.count);

                        cbvaultVaultReadEvent.resultCode = 0;
                    }
                    catch (IOException e)
                    {
                        System.out.println(String.format("vaultRead : %s", e.getMessage()));
                        cbvaultVaultReadEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                    }
                }

                public void vaultSetSize(CbvaultVaultSetSizeEvent cbvaultVaultSetSizeEvent) {
                    RandomAccessFile stream = (RandomAccessFile) gchandle.FromIntPtr(String.valueOf(cbvaultVaultSetSizeEvent.vaultHandle)).getTarget();
                    try {
                        stream.setLength(cbvaultVaultSetSizeEvent.newSize);
                        cbvaultVaultSetSizeEvent.resultCode = 0;
                    }
                    catch (IOException e)
                    {
                        System.out.println(String.format("vaultSetSize : %s", e.getMessage()));
                        cbvaultVaultSetSizeEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                    }
                }

                public void vaultWrite(CbvaultVaultWriteEvent cbvaultVaultWriteEvent) {
                    RandomAccessFile stream = (RandomAccessFile) gchandle.FromIntPtr(String.valueOf(cbvaultVaultWriteEvent.vaultHandle)).getTarget();
                    try {
                        stream.seek(cbvaultVaultWriteEvent.offset);
                        byte[] buff = new byte[cbvaultVaultWriteEvent.buffer.capacity()];
                        cbvaultVaultWriteEvent.buffer.get(buff);
                        stream.write(buff);
                        cbvaultVaultWriteEvent.count = buff.length;

                        cbvaultVaultWriteEvent.resultCode = 0;
                    }
                    catch (IOException e)
                    {
                        System.out.println(String.format("vaultWrite : %s", e.getMessage()));
                        cbvaultVaultWriteEvent.resultCode = -42; // VAULT_ERR_CALLBACK_MODE_FAILURE
                    }
                }
            });
    ...
    static public int AddFiles(String VaultName, List<String> FileNames)
    {
        ...
        try
        {
            Method method = callbackvault.class.getMethod("AddFileObject", String.class, boolean.class, Cbvault.class);
    ...
    static public int DeleteFiles(String VaultName, List<String> FileNames)
    {
        ...
        try
        {
            Method method = callbackvault.class.getMethod("DeleteFileObject", long.class, Cbvault.class);
            for (String FileName : FileNames)
            {
    ...
    static public int ExtractFiles(String VaultName, List<String> FileNames)
    {
        ...
        try
        {
            Method method = callbackvault.class.getMethod("ExtractFileObject", long.class, Cbvault.class);
            for (String FileName : FileNames)
            {
                Result = IterateVault(FileName, method, true, Vault);
                if (Result != 0)
    ...
    static public int ListFiles(String VaultName, List<String> FileNames)
    {
        ...
        try
        {
            Method method = callbackvault.class.getMethod("ListFile", long.class, Cbvault.class);
            for (String FileName : FileNames)
            {
                Result = IterateVault(FileName, method, true, Vault);
                if (Result != 0)
    ...
    static public int TestFiles(String VaultName, List<String> FileNames)
    {
        ...
        try
        {
            Method method = callbackvault.class.getMethod("TestFile", long.class, Cbvault.class);
            for (String FileName : FileNames)
            {
    ...
    static private String ConvertRelativePathToAbsolute(String path) {
    ...
    static private boolean isDriveLetter(String path)
    {
        ...
        try
        {
            switch (InputCommand)
            {
                case 'a':
                    ...
                    CmdFunc = callbackvault.class.getMethod("AddFiles", String.class, List.class);
                    break;
                case 'd':
                    TheCommand = Commands.Delete;
                    CmdFunc = callbackvault.class.getMethod("DeleteFiles", String.class, List.class);
                    break;
                case 'e':
                    TheCommand = Commands.Extract;
                    CmdFunc = callbackvault.class.getMethod("ExtractFiles", String.class, List.class);
                    break;
                case 'l':
                    TheCommand = Commands.List;
                    CmdFunc = callbackvault.class.getMethod("ListFiles", String.class, List.class);
                    break;
                case 't':
                    TheCommand = Commands.Test;
                    CmdFunc = callbackvault.class.getMethod("TestFiles", String.class, List.class);
                    break;
                case 'x':
