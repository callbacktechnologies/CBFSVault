...
def cb_vault_open(params: CBVaultVaultOpenEventParams):
    global context_holder, holder_lock, holder_key_counter
    context = None

    # check if such file is already opened
    if params.vault_handle != -1:
        with holder_lock:
            try:
                context = context_holder.get(params.vault_handle)
    ...
    params.vault_handle = -1
    try:
        if params.open_mode == VAULT_OM_CREATE_NEW:
            if params.read_only:
                params.result_code = VAULT_ERR_INVALID_PARAMETER
                return
            if os.path.isfile(params.vault):
                params.result_code = VAULT_ERR_VAULT_FILE_ALREADY_EXISTS
                return
            open_flag = os.O_RDWR | os.O_CREAT
            open_ch = "wb"
        if params.open_mode == VAULT_OM_CREATE_ALWAYS:
            if params.read_only:
                params.result_code = VAULT_ERR_INVALID_PARAMETER
                return
            open_flag = os.O_RDWR | os.O_CREAT
            open_ch = "wb"
        if params.open_mode == VAULT_OM_OPEN_EXISTING:
            if not os.path.isfile(params.vault):
                params.result_code = VAULT_ERR_VAULT_FILE_DOES_NOT_EXIST
                return
            if params.read_only:
                ...
                open_flag = os.O_RDWR
                open_ch = "r+b"
        if params.open_mode == VAULT_OM_OPEN_ALWAYS:
            if not os.path.isfile(params.vault):
                if params.read_only:
                    params.result_code = VAULT_ERR_INVALID_PARAMETER
                    return
                ...
                if params.read_only:
                    ...
                    open_flag = os.O_RDWR
                    open_ch = "r+b"
        fd = os.open(params.vault, open_flag | os.O_BINARY)
        hfile = os.fdopen(fd, open_ch)

        # create a context
        context = FileContext(params.vault)
        context.hfile = hfile
        context.fd = fd
        context.increment()
        ...
        params.vault_handle = context.id
        params.result_code = 0
    except OSError as ex:
        params.result_code = ex.winerror
    except Exception as ee:
        params.result_code = VAULT_ERR_CALLBACK_MODE_FAILURE


def cb_vault_close(params: CBVaultVaultCloseEventParams):
        ...
        if context is not None:
            with context.counter_lock:
                ...
                if context.counter == 0:
                    ...
                    with holder_lock:
                        context_holder.pop(params.vault_handle)
                    params.result_code = 0
    except OSError as ex:
        params.result_code = ex.winerror
    except Exception as ee:
...
def cb_vault_read(params: CBVaultVaultReadEventParams):
    global context_holder, holder_lock
    context = None
    ...
    try:
        if context is not None:
            with context.counter_lock:
                afile = context.hfile
                if afile != None:
                    buf = bytearray(params.count)
                    afile.seek(params.offset)
                    bread = afile.readinto(buf)
                    if bread != params.count:
                        return

                    # this is how we get a pointer to the array
                    cbuf = ctypes.c_uint8 * (bread)
                    pbuf = cbuf.from_buffer(buf, 0)
                    # now copy the data
                    ctypes.memmove(params.buffer, pbuf, bread)
                    params.result_code = 0

    except OSError as ex:
        params.result_code = ex.winerror
    except Exception as ee:
        # the result code is already set above
        pass


def cb_vault_write(params: CBVaultVaultWriteEventParams):
    global context_holder, holder_lock
    context = None
        ...
        if context is not None:
            with context.counter_lock:
                ...
                if afile != None:
                    buf = bytearray(params.count)
                    # this is how we get a pointer to the array
                    cbuf = ctypes.c_uint8 * (params.count)
                    pbuf = cbuf.from_buffer(buf, 0)
                    # now copy the data
                    ctypes.memmove(pbuf, params.buffer, params.count)
                    afile.seek(params.offset)
                    bwritten = afile.write(buf)
                    if bwritten == params.count:
                        params.result_code = 0
...
def cb_vault_get_size(params: CBVaultVaultGetSizeEventParams):
    global context_holder, holder_lock
    context = None
        ...
        if context is not None:
            with context.counter_lock:
                ...
                if afile != None:
                    ...
                    params.size = afile.tell()
                    afile.seek(savepos, os.SEEK_SET)
                    params.result_code = 0
    except OSError as ex:
        params.result_code = ex.winerror
    except Exception as ee:
...
def cb_vault_set_size(params: CBVaultVaultSetSizeEventParams):
    global context_holder, holder_lock
    context = None
    ...
    try:
        if context is not None:
            with context.counter_lock:
                afile = context.hfile
                if afile != None:
                    afile.seek(params.new_size, os.SEEK_SET)
                    afile.truncate()
                    params.result_code = 0
    except OSError as ex:
        params.result_code = ex.winerror
...
def cb_vault_flush(params: CBVaultVaultFlushEventParams):
    global context_holder, holder_lock
    context = None
...
def cb_vault_delete(params: CBVaultVaultDeleteEventParams):
    try:
        if os.path.isfile(params.vault):
            os.remove(params.vault)
            params.result_code = 0
        else:
            params.result_code = VAULT_ERR_INVALID_PARAMETER
    except OSError as ex:
...
def open_vault(must_exist):
    ...
    vault.callback_mode = True
    vault.on_vault_open = cb_vault_open
    vault.on_vault_close = cb_vault_close
    vault.on_vault_get_size = cb_vault_get_size
    vault.on_vault_set_size = cb_vault_set_size
    vault.on_vault_flush = cb_vault_flush
    vault.on_vault_read = cb_vault_read
    vault.on_vault_write = cb_vault_write
    vault.on_vault_delete = cb_vault_delete
...
def get_directories(path):
    ...
    res = None
    try:
        res = [entry for entry in os.listdir(path)
