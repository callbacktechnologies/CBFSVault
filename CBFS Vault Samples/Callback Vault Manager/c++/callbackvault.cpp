...
#define mychsize _chsize_s
#else
#define mylseek lseek64
#define mychsize chsize
#endif
...
class CBVaultEx : public CBVault
{
public:
    ~CBVaultEx() 
    {
    ...
    {
        int ret;
        std::string FileName = ComponentStringToStdString(e->Vault);
        ret = remove(FileName.c_str());
        e->ResultCode = (ret != -1) ? 0 : EFAULT;
        return e->ResultCode;
    }
    ...
    virtual int FireVaultGetSize(CBVaultVaultGetSizeEventParams *e)
    {
        ...
        if (fstat((const int) e->VaultHandle, &fileStat) == 0)
          *(e->pSize) = fileStat.st_size;
        else
          e->ResultCode = errno;
        return e->ResultCode;
    }
    ...
    virtual int FireVaultOpen(CBVaultVaultOpenEventParams *e) 
    {
        int fd;
            ...
            if (e->OpenMode == cbvConstants::VAULT_OM_CREATE_NEW)
                openFlags = O_CREAT | O_EXCL;
            else
            if (e->OpenMode == cbvConstants::VAULT_OM_CREATE_ALWAYS)
                openFlags = O_CREAT | O_TRUNC;
            else
            if (e->OpenMode == cbvConstants::VAULT_OM_OPEN_EXISTING)
                openFlags = 0;
            if (e->OpenMode == cbvConstants::VAULT_OM_OPEN_ALWAYS)
                openFlags = O_CREAT;
        
        std::string FileName = ComponentStringToStdString(e->Vault);
        
        fd = open(FileName.c_str(), openFlags | (e->ReadOnly ? O_RDONLY : O_RDWR) 
#ifdef _WIN32
          | O_BINARY
#endif
          , S_IREAD | S_IWRITE);
        *(e->pVaultHandle) = fd;
        e->ResultCode = (fd != -1) ? 0 : ENFILE;
        return e->ResultCode;
    }
    ...
    {
      LONG64 ret;
        ret = mylseek((int)(e->VaultHandle), e->Offset, SEEK_SET);
        if (ret != -1)
            ret = read((int)e->VaultHandle, e->Buffer, e->Count);
        e->ResultCode = (ret != -1) ? 0 : EIO;
        return e->ResultCode;
    }
    ...
    {
#ifdef _WIN32
        if (mychsize((int)e->VaultHandle, e->NewSize) != 0)
#else
        if (ftruncate(e->VaultHandle, e->NewSize) != 0)
#endif
          e->ResultCode = errno;
        return e->ResultCode;
    }
    ...
    {
      LONG64 ret;
        ret = mylseek((int)e->VaultHandle,
          e->Offset, 
          SEEK_SET);
        if (ret != -1)
            ret = write((int)e->VaultHandle, e->Buffer, e->Count);
        e->ResultCode = (ret != -1) ? 0 : EIO;
        return e->ResultCode;
    }
};
